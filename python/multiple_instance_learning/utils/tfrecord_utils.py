...
def parse_bag(record, instance_size, num_labels):
    ...
    x = tf.cast(x, tf.float32)  # tf.float16 or tf.float32 # TODO: Forgot float16 cast here eariler?
    print(x.shape)

    y = tf.io.decode_raw(image_features.get('Y'), tf.uint8)
    y = tf.reshape(y, (num_labels, ))
    y = tf.cast(y, tf.float32)  # TODO: Cast to float16? I used float32 earlier here...
